...
def Final_xy(charge_list, x, y, Output, potentials, positions, count, gradients):
    potential_parts = []
...
gradients=[gradient_1,gradient_2,gradient_3,gradient_4,gradient_5,gradient_6,]
            ...
            for x in range(play_space.left, play_space.right-4, 20):
                for y in range(play_space.top, play_space.bottom-4, 20):
                    with concurrent.futures.ProcessPoolExecutor() as executor:
                        ...
                        p = executor.submit(Final_xy, charge_list, x, y, Output, potentials, positions, count, gradients)
                        
                        print(p.result())
                        pygame.draw.line(screen, p.result()[0], [x,y], p.result()[1][count][1], width=2)
                        triangle=[p.result()[1][count][1], [p.result()[1][count][1][0]+3*(p.result()[2][1]-p.result()[2][0]),p.result()[1][count][1][1]+3*(-p.result()[2][1]-p.result()[2][0])], [p.result()[1][count][1][0]+3*(-p.result()[2][1]-p.result()[2][0]),p.result()[1][count][1][1]+3*(-p.result()[2][1]+p.result()[2][0])]]
                        pygame.draw.polygon(screen, p.result()[0], triangle)
